...
let s2 = s1;
```
![1721031795504](img/所有权/trpl04-04.svg)
...
### Dangling References
`rust`编译器保证永不会有dangling reference
```rust
fn main() {
...
`slice`长度为`end_index` - `start_index`

若想从`index0`开始，可以省略`start_index`,如`&s[..end_index]`;同样若想包含`String`的最后一个字符,可省略`end_index`,如`&s[start_index..]`（等于`&s[start_index..s.len()]`）。若取整个`String`，可以全部省略`&s[..]`


`string slice`写作`&str`，如
```rust
...
let s = "Hello, world!";
```
`s` 实际是一个`&str`类型，这也解释了为什么字符串字面量不可变，因为`&str`是不可变引用 `immutable reference`
...
## 总结
ownership,borrowing,和slice在编译期确保了rust的内存安全
